...
const MyAccount = () => {
  ...
  const { data: addressesResponse, refetch: refetchAddresses } = useQuery({
    ...
    queryFn: () => marketplaceService.getClientAddresses(user?.id || ""),
    enabled: !!user?.id,
  });

  const { data: mainAddressResponse } = useQuery({
    queryKey: ["main-address", user?.id],
    queryFn: () => marketplaceService.getMainAddress(user?.id || ""),
    enabled: !!user?.id,
  });

  const addresses = addressesResponse?.data || [];
  const mainAddress = mainAddressResponse?.data;
      ...
      <section className="overflow-hidden py-20 bg-gray-2">
        <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
              ...
              <div className="flex xl:flex-col">
                <div className="hidden lg:flex flex-wrap items-center gap-5 py-6 px-4 sm:px-7.5 xl:px-9 border-r xl:border-r-0 xl:border-b border-gray-3">
                  ...
                  <div>
                    ...
                    <p className="text-custom-xs">Member since {profile?.created_at ? new Date(profile.created_at).getFullYear() : '...'}</p>
                  </div>
                </div>
          ...
          <!--== user dashboard content start ==--> */}
            ...
            >
              <p className="text-dark">
                Hello {profile?.name || user?.name || 'User'} (not {profile?.name || user?.name || 'User'}?
                <button
                  onClick={() => {/* logout logic if needed */ }}
              ...
              {mainAddress && (
                <div className="mt-8 pt-8 border-t border-gray-3">
                  <h4 className="text-lg font-bold text-dark mb-4">Default Shipping Address</h4>
                  <div className="bg-gray-1 rounded-xl p-6 border border-gray-3 inline-block min-w-[300px]">
                    <p className="font-semibold text-dark">{mainAddress.street_address || mainAddress.streetAddress}</p>
                    <p className="text-sm text-dark-5 mt-1">
                      {mainAddress.city}, {mainAddress.region_id || mainAddress.regionId}
                    </p>
                    <button
                      onClick={() => setActiveTab("addresses")}
                      className="text-blue text-sm font-medium mt-4 hover:underline flex items-center gap-1"
                    >
                      Barcha manzillar
          ...
          <!-- addresses tab content start --> */}
            <div
              className={`flex-col gap-6 ${activeTab === "addresses" ? "flex" : "hidden"
                }`}
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {(addresses as any[]).map((addr) => (
                  <div key={addr.id} className="bg-white border border-gray-3 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex items-center justify-between py-4 px-6 border-b border-gray-3 bg-gray-1">
                      <p className="font-semibold text-dark">
                        {(addr.is_main || addr.isMain) ? "Asosiy manzil" : "Qo'shimcha manzil"}
                      </p>
                        ...
                        <button
                          ...
                          onClick={async () => {
                            if (confirm("Ushbu manzilni o'chirmoqchimisiz?")) {
                              await marketplaceService.deleteClientAddress(addr.id);
                              refetchAddresses();
                            }
                          }}
                        >
                    ...
                    <div className="p-6">
                      <div className="space-y-2">
                        <p className="text-dark font-medium leading-relaxed">{addr.street_address || addr.streetAddress}</p>
                        <p className="text-sm text-dark-5">
                          {addr.city}{(addr.region_id || addr.regionId) && `, Region ID: ${addr.region_id || addr.regionId}`}
                        </p>
                        {(addr.apartment || addr.floor) && (
                          <p className="text-sm text-dark-5">
                            {addr.apartment && `Kvartira: ${addr.apartment}`}{addr.apartment && addr.floor && ", "}{addr.floor && `Qavat: ${addr.floor}`}
                          </p>
                        )}
                        {!(addr.is_main || addr.isMain) && (
                          <button
                            onClick={async () => {
                              try {
                                await marketplaceService.setMainAddress(addr.id);
                                refetchAddresses();
                              } catch (error) {
                                console.error("Failed to set main address:", error);
                              }
                            }}
                            className="text-blue text-xs font-semibold hover:underline mt-3"
                          >
                ...
                <button
                  className="group bg-gray-1 border-2 border-dashed border-gray-3 rounded-xl flex items-center justify-center p-8 hover:border-blue hover:bg-blue/5 transition-all min-h-[180px]"
                  onClick={() => {
                ...
                >
                  <div className="text-center">
                    <div className="w-12 h-12 rounded-full bg-white border border-gray-3 flex items-center justify-center mx-auto mb-3 group-hover:border-blue transition-all shadow-sm">
                      <svg className="text-dark group-hover:text-blue transition-colors" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                        <line x1="12" y1="5" x2="12" y2="19"></line>
                        <line x1="5" y1="12" x2="19" y2="12"></line>
                      </svg>
                    </div>
                    <p className="font-semibold text-dark group-hover:text-blue transition-colors">Yangi manzil qo'shish</p>
                  </div>
                </button>
              </div>
            </div>
                  ...
                  <div className="flex flex-col lg:flex-row gap-5 sm:gap-8 mb-5">
                    <div className="w-full">
                      ...
                      <input
                        ...
                        defaultValue={profile?.name?.split(' ')[0] || user?.name?.split(' ')[0] || ''}
                    ...
                    <div className="w-full">
                      ...
                      <input
                        ...
                        defaultValue={profile?.name?.split(' ').slice(1).join(' ') || user?.name?.split(' ').slice(1).join(' ') || ''}
                        className="rounded-md border border-gray-3 bg-gray-1 placeholder:text-dark-5 w-full py-2.5 px-5 outline-none duration-200 focus:border-transparent focus:shadow-input focus:ring-2 focus:ring-blue/20"
                      />
                    </div>
                  </div>
